...
func (s *BlockchainClient) ReadGenesis() *core.Genesis {
	...
	defer file.Close()
	log.Println("Found genesis file, reading initial distribution from ", s.GenesisPath)
...
func (s *BlockchainClient) GetGenesisAllocation() map[string]big.Int {
	...
	log.Println("Genesis allocation: ", genesis.Alloc)

	m := map[string]big.Int{}
	for address, account := range genesis.Alloc {
		var balance big.Int = *account.Balance
		log.Println("Allocated account: ", address.Hex(), balance)
